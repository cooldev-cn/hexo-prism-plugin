...
function PrismPlugin(data) {
    ...
    data.content = data.content.replace(regex, (origin,classes, lang, code) => {
        const lineNumbers = line_number ? 'line-numbers' : '';
        //lang没指定的话,默认为none 2019-4-11 buyucoder
        lang = typeof lang=='undefined'?'none':lang;
        const startTag = `<pre class="${lineNumbers} language-${lang}"><code class="language-${lang}">`;
        const endTag = `</code></pre>`;
        // code = unescape(code);
        let parsedCode = '';
        if (Prism.languages[lang]) {
            //指定语言的要unescape,不然输出的是转义的 2019-4-11 buyucoder
            code = unescape(code);
            parsedCode = Prism.highlight(code, Prism.languages[lang]);
        } else {
            //没指定语言的不能unescape,不然遇到js,html代码会被执行 2019-4-11 buyucoder
            parsedCode = code;
        }
